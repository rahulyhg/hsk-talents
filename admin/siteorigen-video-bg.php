...
class Background_Videos_For_SiteOrigin {
	...
	public function video_after_row( $output, $grid_item, $grid_attributes ) {
		...
		if( $use_background_video ) {
			...
			*/
			...
			// Pattern stuff
			$patterns_path = HSK_PLUGIN_PATH . 'admin/assests/patterns/';
	...
	public function custom_row_style_group( $groups, $post_id, $args ) {
		...
		$groups['bgvideo'] = array(
			'name' => esc_html__('Background Video', 'hsktalents'),
			'priority' => 25
		);
	...
	public function custom_row_style_fields( $fields ) {
		...
		$fields['mp4_url'] = array(
			'name'        => esc_html__('Mp4 URL', 'hsktalents'),
			'type'        => 'url',
			'group'       => $group,
			'description' => esc_html__('Required file type. We recommend using <a href="https://html5backgroundvideos.com/converter?utm_source=SO%20Video%20Backgrounds%20Addon&utm_medium=Text%20Link&utm_content=Files%20section%20of%20background%20tab&utm_campaign=WordPress%20Plugins" target="_blank">this converter</a> to generate your background video files.', 'hsktalents'),
			'priority'    => $order ++,
			'default'     => ''
		);
		...
		$fields['webm_url'] = array(
			'name'        => esc_html__('WebM URL', 'hsktalents'),
			'type'        => 'url',
			'group'       => $group,
			'description' => esc_html__('Recommended file type.', 'hsktalents'),
			'priority'    => $order ++,
			'default'     => ''
		);
		...
		$fields['ogg_url'] = array(
			'name'        => esc_html__('Ogg URL', 'hsktalents'),
			'type'        => 'url',
			'group'       => $group,
			'description' => esc_html__('Optional file type.', 'hsktalents'),
			'priority'    => $order ++,
			'default'     => ''
		);
/*
		...
		$fields['poster_image'] = array(
			'name'        => esc_html__('Poster / fallback image', 'hsktalents'),
			'type'        => 'image',
			'group'       => $group,
			'description' => esc_html__('This image will be used on devices that don\'t support background video, and will be displayed while the video is loading. We recommend a high-quality screenshot of one of the first few frames. For best results, you should also set this as the background image for this row.', 'hsktalents'),
			'priority'    => $order ++,
			'default'     => ''
		);
*/
		// Poster (url)
		$fields['poster_image'] = array(
			'name'        => esc_html__('Poster / fallback image URL', 'hsktalents'),
			'type'        => 'url',
			'group'       => $group,
			'description' => esc_html__('This image will be used on devices that don\'t support background video, and will be displayed while the video is loading. We recommend a high-quality screenshot of one of the first few frames. For best results, you should also set this as the background image for this row.', 'hsktalents'),
			'priority'    => $order ++,
		...
		$fields['video_overlay'] = array(
			'name'        => esc_html__('Video Overlay', 'hsktalents'),
			'type'        => 'select',
			'group'       => $group,
			'description' => esc_html__('An overlay can help to provide contrast with your row content, or disguise low quality video.', 'hsktalents'),
			'priority'    => $order ++,
			'options'     => array(
								'none'          => 'No overlay',
								'solid'         => 'Color',
		...
		$fields['overlay_opacity'] = array(
			'name'        => esc_html__('Overlay Opacity', 'hsktalents'),
			'type'        => 'text',
			'group'       => $group,
			'description' => esc_html__('Enter a number between 0 and 99. (Only relevant if you choose an overlay type)', 'hsktalents'),
			'priority'    => $order ++,
			'default'     => ''
		);
		...
		$fields['overlay_color'] = array(
			'name'        => esc_html__('Overlay Color', 'hsktalents'),
			'type'        => 'color',
			'group'       => $group,
			'description' => esc_html__('(Only relevant if you choose a colored overlay)', 'hsktalents'),
			'priority'    => $order ++,
			'default'     => ''
		);
		...
		$fields['overlay_pattern'] = array(
			'name'        => esc_html__('Overlay Pattern', 'hsktalents'),
			'type'        => 'select',
			'group'       => $group,
			'description' => esc_html__('(Only relevant if you set overlay type to a pattern). Patterns are especially good at disguising low quality video. See a demo of these patterns <a href="https://html5backgroundvideos.com/pattern-overlays?utm_source=SO%20Video%20Backgrounds%20Addon&utm_medium=Text%20Link&utm_content=Pattern%20dropdown%20description&utm_campaign=WordPress%20Plugins" target="_blank">here</a>.', 'hsktalents'),
			'priority'    => $order ++,
		...
		$fields['fade_in'] = array(
			'name'        => esc_html__('Fade in on start?', 'hsktalents'),
			'type'        => 'checkbox',
			'default'     => 'true',
			'group'       => $group,
			'description' => esc_html__('Fading the video in on start can help avoid distracting the user.', 'hsktalents'),
			'priority'    => $order ++
		);

		// Pause after
		$fields['pause_after'] = array(
			'name'        => esc_html__('Pause video after...', 'hsktalents'),
			'type'        => 'text',
			'default'     => '120',
			'group'       => $group,
			'description' => esc_html__('Enter a number of seconds to play before pausing, or 0 for no pause. We recommend pausing after a while and fading out, to reduce your users\' power consumption.', 'hsktalents'),
			'priority'    => $order ++
		);

		// Pause button
		$fields['pause_play_button'] = array(
			'name'        => esc_html__('Add pause/play button?', 'hsktalents'),
			'type'        => 'checkbox',
			'default'     => 'true',
			'group'       => $group,
			'description' => esc_html__('It\'s a good idea to allow your user to pause the video if they wish.', 'hsktalents'),
			'priority'    => $order ++
		);

		// Pause X Position
		$fields['pauseplay_xpos'] = array(
			'name'        => esc_html__('Pause/play button X position', 'hsktalents'),
			'type'        => 'select',
			'group'       => $group,
			'description' => esc_html__('(Only relevant if you check "Add pause/play button")', 'hsktalents'),
			'priority'    => $order ++,
			'options'     => array(
								'left' => 'Left',
								'center' => 'Center',
		...
		$fields['pauseplay_ypos'] = array(
			'name'        => esc_html__('Pause/play button Y position', 'hsktalents'),
			'type'        => 'select',
			'group'       => $group,
			'description' => esc_html__('(Only relevant if you check "Add pause/play button")', 'hsktalents'),
			'priority'    => $order ++,
	...
	public function bgvideo_prebuilt_layouts( $layouts ){
		...
		$layouts['example-background-video'] = array (
			'name' => esc_html__('Example Background Video', 'hsktalents'),
        	'description' => esc_html__('A pre-built background video layer with a simple text widget.', 'hsktalents'),
		  	'widgets' => 
			  array (
			    0 => 
			    array (
	...
	public function add_scripts_and_styles() {
		wp_register_style( 'jquery-background-video',  HSK_PLUGIN_PATH . 'admin/assests/jquery.background-video.css' );
		wp_register_style( 'hsk-video-background',  HSK_PLUGIN_PATH . 'admin/assests/video_background.css' );
		wp_enqueue_style( 'jquery-background-video' );
		wp_enqueue_style( 'hsk-video-background' );
		// If you need any javascript files on front end, here is how you can load them.
		wp_register_script( 'jquery-background-video',HSK_PLUGIN_PATH . 'admin/assests/jquery.background-video.js', array('jquery'), '1.1.1', true );
